...
must_haves:
  ...
  artifacts:
    - path: "src/Imagile.Framework.Core/Attributes/DoNotUpdateAttribute.cs"
      provides: "Validation marker attribute for excluding properties from updates"
...
<task type="auto">
  <name>Task 2: Final verification and package validation</name>
  <files></files>
  <action>
Perform final verification of the complete Core package:

1. Verify all 8 attribute files exist:
   - AssociatedAttribute.cs (base, not sealed)
   - RequiresAttribute.cs (inherits, not sealed)
   - IncludesAttribute.cs (inherits, not sealed)
   - CategoryAttribute.cs (sealed)
   - CountAttribute.cs (sealed)
   - NativeNameAttribute.cs (sealed)
   - HostedAttribute.cs (sealed)
   - DoNotUpdateAttribute.cs (sealed)

2. Verify zero dependencies:
   - grep PackageReference src/Imagile.Framework.Core/Imagile.Framework.Core.csproj
...
4. Create NuGet package:
   - dotnet pack src/Imagile.Framework.Core -c Release
   - Must produce Imagile.Framework.Core.*.nupkg

5. Verify XML documentation generation:
   - Check bin/Release/net10.0/ for Imagile.Framework.Core.xml
  </action>
  <verify>
1. dotnet build Imagile.Framework.sln -c Release succeeds
2. dotnet pack src/Imagile.Framework.Core -c Release produces .nupkg
3. All 8 attribute files exist in src/Imagile.Framework.Core/Attributes/
4. grep PackageReference src/Imagile.Framework.Core/*.csproj returns 0 matches
5. bin/Release/net10.0/Imagile.Framework.Core.xml exists (documentation file)
  </verify>
  <done>Core package is complete with all 8 attributes, zero dependencies, and generates valid NuGet package with XML documentation.</done>
</task>
