...
must_haves:
  truths:
    - "Developer can access IApplicationInsights interface that defines all tracking methods"
    - "Developer can configure SDK options via Config model with all Application Insights settings"
    - "Developer can create telemetry objects (Event, Exception, Trace, Metric, PageView) for custom tracking"
    - "Developer can set severity levels (Verbose, Information, Warning, Error, Critical) matching App Insights"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    5. **RENAME abbreviated files**: CookieMgr.cs → CookieManager.cs (update class name and all references accordingly)
    ...
    - CookieManager.cs (cookie manager wrapper)
  </action>
  <verify>
